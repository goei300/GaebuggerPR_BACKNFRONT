...
//        responseData.put("process_Guide_Violate",5);
...
//        return new ResponseEntity<>(responseData, HttpStatus.OK);
    ...
    public static List<Map<String, Object>> createProcessIssues() {
        ...
        try {
            // JSON 파일 로드
            InputStream inputStream = TypeReference.class.getResourceAsStream("/testFile.json");
            // JSON 파일을 List<Map<String, Object>> 형태로 변환
            processIssues = mapper.readValue(inputStream, new TypeReference<List<Map<String, Object>>>(){});
        } catch (Exception e) {
            e.printStackTrace();
        }
